...
control MyIngress(inout headers hdr,
    ...
    register<bit<48>> (REGISTER_SIZE) tm_stamp;
...
//digest packet
    ...
    apply {
        if (hdr.ipv4.isValid()){
            ...
            // ctr.read(meta.tmp_store_ctr, 0);
            // meta.tmp_store_ctr = meta.tmp_store_ctr+ 1;
            // ctr.write(0, meta.tmp_store_ctr);


            // if(meta.tmp_store_ctr==1){
            //     tm_stamp.write(0, standard_metadata.ingress_global_timestamp);
            // }


            // if(meta.tmp_store_ctr==70){
                

            //     tm_stamp.write(1, standard_metadata.ingress_global_timestamp-meta.tmp_time);


            // }


            tm_stamp.read(meta.tmp_time,0);
            if(meta.tmp_time==0){
                tm_stamp.write(0, standard_metadata.ingress_global_timestamp);
            }
            ...
            if (hdr.tcp.isValid()){
                ...
                if ( (meta.counter_one > PACKET_THRESHOLD && meta.counter_two > PACKET_THRESHOLD && standard_metadata.ingress_global_timestamp-meta.tmp_time<6) ){
                    drop_table.apply();
                    return;
                }
